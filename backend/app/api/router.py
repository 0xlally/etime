"""API Router - Aggregates all API routes"""
from fastapi import APIRouter
from .endpoints import health, auth, users, categories, sessions, stats, heatmap, targets, evaluations, notifications, admin, time_traces, reviews, groups, quick_start_templates, calendar_tasks

# Create main API router
api_router = APIRouter()

# Include health check endpoint
api_router.include_router(health.router, tags=["health"])

# Include authentication endpoints
api_router.include_router(auth.router, prefix="/auth", tags=["auth"])

# Include user endpoints
api_router.include_router(users.router, prefix="/users", tags=["users"])

# Include category endpoints
api_router.include_router(categories.router, prefix="/categories", tags=["categories"])

# Include session endpoints
api_router.include_router(sessions.router, prefix="/sessions", tags=["sessions"])

# Include quick start template endpoints
api_router.include_router(quick_start_templates.router, prefix="/quick-start-templates", tags=["quick-start-templates"])

# Include stats endpoints
api_router.include_router(stats.router, prefix="/stats", tags=["stats"])

# Include heatmap endpoints
api_router.include_router(heatmap.router, prefix="/heatmap", tags=["heatmap"])

# Include work targets endpoints
api_router.include_router(targets.router, prefix="/targets", tags=["targets"])

# Include evaluations endpoints (primary)
api_router.include_router(evaluations.router, prefix="/evaluations", tags=["evaluations"])
# Backward compatibility for older frontend build that called /work-evaluations
api_router.include_router(evaluations.router, prefix="/work-evaluations", tags=["evaluations-legacy"])

# Include notifications endpoints
api_router.include_router(notifications.router, prefix="/notifications", tags=["notifications"])

# Include time trace endpoints
api_router.include_router(time_traces.router, prefix="/time-traces", tags=["time-traces"])

# Include review endpoints
api_router.include_router(reviews.router, prefix="/reviews", tags=["reviews"])

# Include planner/calendar task endpoints
api_router.include_router(calendar_tasks.router, prefix="/calendar-tasks", tags=["calendar-tasks"])

# Include group endpoints
api_router.include_router(groups.router, prefix="/groups", tags=["groups"])

# Include admin endpoints
api_router.include_router(admin.router, prefix="/admin", tags=["admin"])

# TODO: Add more routers as you develop your application
# Example:
# from .endpoints import items
# api_router.include_router(items.router, prefix="/items", tags=["items"])
